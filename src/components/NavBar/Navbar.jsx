...
const Navbar = () => {
  ...
  const handleMouseEnter = (item) => {
    setDropdown((prevDropdown) => (prevDropdown === item ? null : item));
  };
  ...
  const handleSearch = () => filterItems(searchQuery);

  const filterItems = (query) => {
    const results = Object.keys(menuItems)
      .map((category) => ({
        category,
        items: menuItems[category].filter((item) =>
